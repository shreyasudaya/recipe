...
class TestAppRoutes(unittest.TestCase):
    ...
    def test_routes(self):
        ...
        # categories
        response = self.client.get('/categories/cuisine/American')
        data = response.data.decode('utf-8')
        assert response.status_code == 200
        assert 'Featured Chinese Recipes' in data

        # recipe
        response = self.client.get('/recipe/6477039caaa13d0be0ed6f6f/Idli')
        data = response.data.decode('utf-8')
        assert response.status_code == 200
        assert 'Idli' in data 

        # pagination
        response = self.client.get('/categories/cuisine/indian')
        data = response.data.decode('utf-8')
        assert response.status_code == 200
        assert 'All Indian Recipes' in data 
